...
class PostListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        print(f'WWWWWWWWWWWWWWWW: {posts_obj}, {len(posts_obj)}')
        following_post_number = 0
        for i in following_list:
            following_post_number += i.following.post.count()

        # recommend random users if there is not many posts to show (or few followings)
        if following_post_number < 5 or len(following_list) < 3:
            ...
            print(f'FFFFFFFFFFFFFFF: {recommended_users}')
            print(f'DDDDDDDDDDDDDDD: {posts_obj}, {len(posts_obj)}')
        return posts_obj
